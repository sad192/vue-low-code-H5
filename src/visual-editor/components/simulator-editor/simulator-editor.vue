<template>
  <div class="simulator-container">
    <div class="simulator-editor">
      <Grid></Grid>
      <!-- v-model默认使用modelValue作为prop，并以 update:modelValue作为对应的事件 -->
      <div ref="editorContainer" class="simulator-editor-content">
        <DraggableTransitionGroup
          v-model:drag="drag"
          v-model="currentPage.blocks"
          class="!min-h-680px"
          draggable=".item-drag"
          :parentlocation="containerLocation"
          :show-item-contain="true"
        >
...
<script lang="tsx" setup>
  ...
  // () => inject<ReturnType<typeof initVisualData>>(injectKey)!
  // 获取的都是在根组件中provide的同一个对象数据
  const { currentPage, setCurrentBlock } = useVisualData();

  const { globalProperties } = useGlobalProperties();

  const drag = ref(false);
  const editorContainer = ref<HTMLBaseElement | null>(null);
  const containerLocation: any = ref(null);

  //获取画布容器的位置
  onMounted(() => {
    const location = editorContainer.value?.getBoundingClientRect();
    containerLocation.value = { x: location?.left!, y: location?.top! };
    console.log('fu', containerLocation);
  });
  /**
